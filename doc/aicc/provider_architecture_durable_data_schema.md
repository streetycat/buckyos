...
ModelRule 只允许模型技术字段：`id/pattern`、`parameter_scale`、`api_types`、`logical_mounts`、`capabilities`、`quality_score`、`version_rules` 引用和可选保守默认价格。禁止 endpoint、认证、protocol adapter、operation、Provider 请求参数、availability 和实例健康状态。
