# AICC Provider 参数配置（讨论稿）

状态：Draft
范围：拆分 Model Driver metadata 与 Provider 渠道逻辑；本文只定义目标边界和示例，不代表当前代码已经实现。
...
1. 哪些 Provider 纳入内置专用 Provider 的发布支持名单。
2. 配置型 Provider 可以选择哪些已实现的通用 adapter，以及选择发生在 Provider Instance 的哪个字段。
3. 配置型 Provider 的参数文件如何接入现有 metadata 云更新、校验和 LKGS 机制。
4. Model Driver 语义 `variants` 与配置型 Provider `variants` 的完整性校验方式。
5. 当前 `provider_driver` 字段如何拆分，避免同时表示协议 adapter、Provider 渠道和 Model Driver。
