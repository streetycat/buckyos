...
| SN AI Provider | AICC `settings.sn-ai-provider`，经 SN 转发到兼容模型服务 | OpenAI Responses-compatible 或 SN 归一响应 | 由 SN AI Provider 能力决定 | 无普通 API key 参数、`device_jwt` / SN 链路可达性、provider instance 命名、usage / trace / free credit 归因 |
